class FetchAddressPositionCargoResponse {
  FetchAddressPositionCargoResponse({
    this.status,
    this.description,
    this.data,
    this.customMessage,
  ...
  FetchAddressPositionCargoResponse.fromJson(Map json) {
    status = json['status'];
...
class AddressPositionResponse {
  ...
  AddressPositionResponse.fromJson(Map json) {
    ...
    if (json['type'] is List) {
      type = (json['type'] as List).firstOrNull ?? '';
    } else {
      type = '';
    }
  }
